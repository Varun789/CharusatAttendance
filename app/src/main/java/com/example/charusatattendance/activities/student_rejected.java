...
import com.example.charusatattendance.adapters.StudentRejectedAdapter;
...
public class student_rejected extends AppCompatActivity {
    ...
    private StudentRejectedAdapter adapter;
    FirebaseAuth mAuth;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_student_rejected);
        ...
        //In approved and rejected going to have same view
        adapter= new StudentRejectedAdapter(getApplicationContext(), forms);
        ...
        studentformsRef.child(uid).orderByChild("form_status").equalTo("rejected").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()){
                    for (DataSnapshot npsnapshot : dataSnapshot.getChildren()){
                        form_pojo l=npsnapshot.getValue(form_pojo.class);
                       Log.i("form rejected", l.getEvent_name());
                        forms.add(l);
